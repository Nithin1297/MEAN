...
// Write a function that filters out dishes with ratings below 4, then returns a string of dish names
// separated by commas.

const getHighRatedDishes = () => {
  return orders
    .filter((dish) => dish.ratings.find((x) => x < 4))
    .map((x) => x.dishName)
    .join(", ");
};
...
// Write a function that slices the first N dishes from the orders array, maps their names,
// and joins them into a single string.

function getFirstNDishNames() {
  return orders
    .slice(0, 3)
...
// Write a function that accepts multiple order IDs, fetches the dish names, and returns
// a formatted string using the rest operator, nullish coalescing, and template literals.
...
// Write a function that accepts any number of food orders and returns a formatted string
// listing their dish names and categories using the rest operator, nullish coalescing, and template literals.

function listOrders(...orderss) {
  return orderss
    .map((order) => `${order.dishName} (${order.category}) `)
    .join();
}
...
const getTotalRatingsForChefs = (orders) => {
  const ratingsPerChef = {};

  orders.forEach((order) => {
    if (!ratingsPerChef[order.chef]) {
      ratingsPerChef[order.chef] = 0;
    }
    ratingsPerChef[order.chef] += order.ratings.length;
  });

  return ratingsPerChef;
};
...
function getOrderDetails(o) {
  //   const {dish{dishName,dishCategory},orderQuantity,orderPrice} = o;
  //   const {dishName,dishCategory} = o.dish
...
function displayOrderSummary(orderId) {
  const order = orders.find((o) => o.id === orderId);
  return order
    ? `${order?.dish?.name ?? "No name"} (${
        order?.dish?.category ?? "No category"
      }) x${order?.quantity ?? "No quantity"} costs $${
        order?.price ?? "No price"
      }`
